...
class AppController extends Controller
{
    ...
    public function home(Request $request)
    {
        try {
            ...
            // Ambil kategori dari request, default ke 'all' jika tidak ada
            $kategoriFilter = $request->input('kategori', 'all');

            // Query produk dengan filter kategori jika diperlukan
            $query = $this->produk->where("mitraId", Auth::user()->karyawan->mitra->id)
                                  ->where("status", 1);
    ...
    public function clear(Request $request)
{
    ...
    Session::forget('keranjangDetail');
    
    // Atau, hapus data keranjang dari database
    DB::table('keranjang')->where('user_id', $user->id)->delete();
